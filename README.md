...
- `monitor01`은 나머지 4대를 감시하는 역할로, node_exporter(서버 상태)와 blackbox_exporter(서비스 상태)를 통해 두 계층을 모두 스크래핑
...
**원인 분석**
```bash
dig web01.lab.local @192.168.56.11   # 응답 없음(타임아웃)
```
...
**검증**
```
probe_success{job="blackbox_dns"} == 1
```
...
```bash
mysql -h db01.lab.local -u testuser -p
# ERROR 2002 (HY000): Can't connect to MySQL server on 'db01.lab.local' (115)
```
...
**검증**
```
probe_success{job="blackbox_tcp"} == 1
```
...
### 05. 신규 VM 추가 시 zone 파일 A 레코드 누락
초기 4대 구성 기준으로 작성된 zone 템플릿에 dns01 자신과 monitor01의 A 레코드가 빠져 있어 Prometheus에서 두 대상이 DOWN으로 표시됐습니다. zone 템플릿에 두 호스트의 A 레코드를 추가해 해결했습니다.

### 06. PTR 추가 후 named 재시작 실패
정방향 zone의 NS 레코드(및 SOA MNAME)가 실제로는 존재하지 않는 `ns01.lab.local`을 가리키고 있어, named가 zone 로드 시 "NS has no address records" 오류로 재시작에 실패했습니다. NS/MNAME을 실제 호스트명(`dns01.lab.local`)으로 수정해 해결했습니다.

### 07. ansible_local 방식에서 인벤토리 그룹 순회의 한계
...
### 08. VM 최초 부팅 직후 shell provisioner에서 간헐적 네트워크 실패
`Vagrantfile`의 `dnf install -y ansible-core`를 실행하는 shell provisioner가 VM 부팅 직후 가장 먼저 실행되는데, 이 시점에 네트워크가 완전히 안정되기 전이라 이미 등록되어 있던 외부 저장소(docker-ce repo) 메타데이터 갱신에서 간헐적으로 실패하는 경우가 있었습니다. 수동으로 같은 명령을 실행하면 정상 동작해 일시적인 타이밍 문제로 판단, 재시도 루프로 감쌌습니다.
...
- Kubernetes/OpenStack 등 다른 인프라 도구와의 통합 검토 (OpenStack은 별도 프로젝트로 진행 중이라 이번 프로젝트와는 분리 유지)
- 장애 시나리오 자동화(카오스 엔지니어링 도구 도입 검토)
